...
-- [FOX FIX] Ультимативный поиск еды (склеиваем все таблицы в одну)
...
-- [FOX FIX] Уравнитель (одна рабочая функция вместо двух косячных)
local function FeedSlimes()
	task.defer(function()
		...
		for _, id in ipairs(equipped) do
			...
			if lvl < minLvl then
				minLvl, targetID, targetName = lvl, id, (inventory[id] and inventory[id].id or id)
			end
		end
...
local function EquipBestManual()
	...
	if not bestUUIDs or #bestUUIDs == 0 then return end

	for _, guid in ipairs(bestUUIDs) do
...
local FEATURES = {
	...
	AutoEquipBest = {
		...
		action = function() EquipBestManual() end
	},
	AutoFeed = {
		kind = "task_loop",
